...
def generate_frames():
    ...
    while True:
        ...
        if detect_start:
            ...
            frame_queue.full() or frame_queue.put(frame)

            # Ambil frame dari output_queue jika tersedia
            if not output_queue.empty():
                cracks = output_queue.get()
                ...
                if max(cracks_id) > now_cracks_id:
                    ...
                    inspection_session_data["count_longitudinal_cracks"] += cracks_id.count(0)
                    inspection_session_data["count_transverse_cracks"] += cracks_id.count(1)
                    inspection_session_data["count_alligator_cracks"] += cracks_id.count(2)
                    inspection_session_data["count_potholes"] += cracks_id.count(3)
